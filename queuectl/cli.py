...
# #     p_enqueue.add_argument("job_json", help="Job specification as JSON string")
...
# #             print(f"  {state}: {count}")
# #         # Worker status based on PID files
...
# #             # Try to interpret as int if possible
# #             v: str | int
...
#     # enqueue
#     p_enqueue = subparsers.add_parser("enqueue", help="Enqueue a new job")
#     # ORIGINAL style: positional JSON (still supported, especially for Linux/tests)
#     p_enqueue.add_argument(
#         "job_json",
#         nargs="?",
#         help="Job specification as JSON string (optional if using --cmd/--id)",
#     )
#     # NEW style: flags for Windows-friendliness
#     p_enqueue.add_argument(
#         "--cmd",
#         dest="job_command",
...
#         try:
#             if args.job_json:
#                 # JSON-style enqueue (original behavior)
#                 job = engine.enqueue_job_from_json(args.job_json)
#             else:
#                 # Flag-style enqueue (new, Windows-friendly)
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    # enqueue
    p_enqueue = subparsers.add_parser("enqueue", help="Enqueue a new job")
    # ORIGINAL style: positional JSON (still supported)
    p_enqueue.add_argument(
        "job_json",
        nargs="?",
        help="Job specification as JSON string (optional if using --cmd/--id)",
    )
    # Flag-style enqueue (Windows-friendly)
    p_enqueue.add_argument(
        "--cmd",
        dest="job_command",
        help="Shell command for the job (alternative to JSON)",
